...
COPY --from=build /lds/target/linked-data-store-memory-0.1-SNAPSHOT.jar /opt/lds/linked-data-store-memory-0.1-SNAPSHOT.jar
...
CMD ["java", "-cp", "/opt/lds/linked-data-store-memory-0.1-SNAPSHOT.jar:/opt/lds/lib/*", "no.ssb.lds.server.Server"]
